...
ReliabilityAnalysis <- function(dataset = NULL, options, perform = "run",
  ...
  results[["reliabilityScale"]] <- .reliabilityScaleTable(resultsAlpha, dataset, options, variables, perform)
  
  if (options$alphaItem || options$gutmannItem || options$itemRestCor || options$meanItem || options$sdItem || options[["mcDonaldItem"]]) {
    results[["reliabilityItemsObj"]] <- list(title="Item Statistics", reliabilityItems=.reliabilityItemsTable(resultsAlpha, options, variables, perform))
  } else {
...
.reliabilityScaleTable <- function (r, dataset, options, variables, perform) {
...
.reliabilityItemsTable <- function (r, options, variables, perform) {
