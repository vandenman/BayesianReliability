...
.BayesianReliabilityDerivedOptions <- function(options) {
  ...
  derivedOptions <- list(
    selectedEstimators  = unlist(options[c("alphaScale", "gutmann2Scale", "glbScale", "mcDonaldScale")]),
    itemDroppedSelected = unlist(options[c("mcDonaldItem", "alphaItem", "gutmann2Item", "glbItem")]),
    namesEstimators     = list(
      tables = c("Cronbach's \u03B1", "Gutmann's \u03BB2", "Greatest Lower Bound", "McDonald's \u03C9"),
  ...
  derivedOptions[["order"]] <- match(c("McDonald's \u03C9", "Cronbach's \u03B1", "Gutmann's \u03BB2",
...
.BayesianReliabilityScaleTable <- function(jaspResults, model, options) {
  ...
  scaleTable$dependOn(options = c("variables", "mcDonaldScale", "alphaScale", "gutmann2Scale", "glbScale", "meanScale",
...
.BayesianReliabilityItemTable <- function(jaspResults, model, options) {
  ...
  itemTable$dependOn(options = c("variables",
                                 "mcDonaldScale", "alphaScale", "gutmann2Scale", "glbScale", "meanScale", "sdScale",
                                 "mcDonaldItem",  "alphaItem",  "gutmann2Item",  "glbItem",  "meanItem",  "sdItem",
                                 "reverseScaledItems"))
  itemTable$addColumnInfo(name = "variable", title = "if item dropped", type = "string")

  idxSelected <- which(itemDroppedSelected)
  for (i in idxSelected) {
    itemTable$addColumnInfo(name = paste0("postMean", i), title = "Posterior Mean", type = "number", overtitle = overTitles[i])
...
.BayesianReliabilityProbTable <- function(jaspResults, model, options) {
  ...
  probTable <- createJaspTable(sprintf("Probability Statistic is Larger than %.2f", options[["probTableValue"]]))
  probTable$dependOn(options = c("variables", "mcDonaldScale", "alphaScale", "gutmann2Scale", "glbScale", "meanScale",
...
.BayesianReliabilityMakeSinglePosteriorPlot <- function(relyFit, scaleCri, i, nms, fixXRange = FALSE, shade = NULL) {
  ...
  if (fixXRange) {
    ...
    d <- stats::density(relyFit$bay$samp[[i]], n = 2^11)
  }
